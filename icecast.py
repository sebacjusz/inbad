        ...
        for k in d:
            if k in ('artist', 'title', 'server_description', 'server_name', 'file'):
                if type(d[k]) == unicode:
                    continue
                try:
                    d[k] = d.get(k, u'').decode('utf-8')
                except UnicodeEncodeError:
                    d[k] = d.get(k, u'').decode('cp1250')
...
class IcecastPoller(service.Service):
    ...
    def startService(self):
        self._slc = task.LoopingCall(self.updateServers).start(self._interval, now=False)
        self._mlc = task.LoopingCall(self.updateMounts).start(self._interval/2, now=False)
    ...
    def updateMounts(self):
        for m in self.mounts:
            ...
            self.m[m] = new
            self.log('relays for %s:' % m+ repr(new['relays']), 2)
